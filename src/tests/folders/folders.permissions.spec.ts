...
test.describe(
  "DELETE /api/2.0/files/folder/:folderId - access control",
  () => {
    test(
      "DELETE /api/2.0/files/folder/:folderId - anonymous cannot delete folder",
      async ({ apiSdk }) => {
    ...
    test(
      "DELETE /api/2.0/files/folder/:folderId - DocSpaceAdmin can delete owner's folder",
      async ({ apiSdk }) => {
        const ownerApi = apiSdk.forRole("owner");
        const { data: roomData } = await ownerApi.rooms.createRoom({
          createRoomRequestDto: {
    ...
    test(
      "DELETE /api/2.0/files/folder/:folderId - ContentCreator can delete their own folder",
      async ({ apiSdk }) => {
        const ownerApi = apiSdk.forRole("owner");
        const { data: roomData } = await ownerApi.rooms.createRoom({
          createRoomRequestDto: {
    ...
    test(
      "DELETE /api/2.0/files/folder/:folderId - RoomAdmin can delete their own folder",
      async ({ apiSdk }) => {
        const ownerApi = apiSdk.forRole("owner");
        const { data: roomData } = await ownerApi.rooms.createRoom({
          createRoomRequestDto: {
        ...
        await test.step("RoomAdmin deletes own folder — folder no longer accessible", async () => {
          await expect(async () => {
            const { status } =
              await roomAdminApi.folders.getFolderByFolderId({ folderId });
            expect(status).not.toBe(200);
          }).toPass({ intervals: [1_000, 2_000, 5_000], timeout: 30_000 });
        });
      },
    ...
    test(
      "DELETE /api/2.0/files/folder/:folderId - Owner can delete RoomAdmin's folder",
      async ({ apiSdk }) => {
        const ownerApi = apiSdk.forRole("owner");
        const { data: roomData } = await ownerApi.rooms.createRoom({
          createRoomRequestDto: {
