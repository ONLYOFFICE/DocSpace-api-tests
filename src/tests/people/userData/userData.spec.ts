...
test.describe("POST /people/remove/start - Start data removal", () => {
  test.fail("BUG 80714: POST /people/remove/start - Room ownership transfers to Owner after deactivated DocSpaceAdmin data removal", async ({
    apiSdk,
  }) => {
    // Create DocSpaceAdmin
  ...
  test.fail("BUG 80714: POST /people/remove/start - Room ownership transfers to Owner after deactivated RoomAdmin data removal", async ({
    apiSdk,
  }) => {
    // Create RoomAdmin
    const { data: roomAdminMember, userData: roomAdminUserData } =
