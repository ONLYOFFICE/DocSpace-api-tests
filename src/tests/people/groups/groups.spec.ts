...
test.describe("POST /group/:id/members - Replace group members", () => {
  test("BUG 79368: POST /group/:id/members - DocSpace admin sets a disabled user replaces all existing members with empty list", async ({
    apiSdk,
  }) => {
    ...
    const { data: disabledUserData } = await apiSdk.addMember("owner", "User");
