...
test.describe("AI Chat - the language a failed reply is reported in", () => {
  ...
  test("BUG 84023: PUT /people/{userId}/culture, POST /api/2.0/ai/ai/send-with-stream - the portal's own gateway refusal is translated", async ({
    apiSdk,
  }) => {
...
//     server-executed `onlyoffice_knowledge_search` tool call. It is auto-run:
...
const KNOWLEDGE_SEARCH_TOOL = "onlyoffice_knowledge_search";
...
/** The `onlyoffice_knowledge_search` results of a reply, as raw JSON strings. */
...
// never calls onlyoffice_knowledge_search at all. First measured sending only
...
test.describe("POST /api/2.0/ai/ai/send-with-stream - AI Chat suggested prompts", () => {
  ...
  // results" bug below: with nothing indexed, `onlyoffice_knowledge_search` is
