...
test.describe("DELETE /api/2.0/files/tags - Input validation", () => {
  ...
  test("BUG 81689: DELETE /api/2.0/files/tags - names array contains empty string returns 400", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { status } = await ownerApi.rooms.deleteCustomTags({
  ...
  test("BUG 81689: DELETE /api/2.0/files/tags - names array contains spaces-only string returns 400", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { status } = await ownerApi.rooms.deleteCustomTags({
  ...
  test("BUG 81689: DELETE /api/2.0/files/tags - names array containing null returns 400 (no 500)", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { status } = await ownerApi.rooms.deleteCustomTags({
...
test.describe("POST /files/roomtemplate - access control", () => {
  ...
  test("BUG 81693: DocSpaceAdmin cannot create a template from someone else's room they have no access to", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
  ...
  test("BUG 81693: User cannot create a template (no source room access)", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
  ...
  test("BUG 81693: Guest cannot create a template", async ({ apiSdk }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
      createRoomRequestDto: {
  ...
  for (const { label, access } of roomAccesses) {
    ...
    test(`BUG 81693: User with ${label} access to source room cannot create template`, async ({
      apiSdk,
    }) => {
      const ownerApi = apiSdk.forRole("owner");
      const { data: roomData } = await ownerApi.rooms.createRoom({
...
test.describe("PUT /files/roomtemplate/public - access control", () => {
  async function createTemplate(
    api: any,
    title: string,
    isPublic = false,
  ...
  test("Owner can update public flag of own template", async ({ apiSdk }) => {
    const ownerApi = apiSdk.forRole("owner");
    const templateId = await createTemplate(
      ownerApi,
      "Autotest SetPublic Owner Own",
    );
    ...
    const { data } = await ownerApi.rooms.getPublicSettings({ id: templateId });
    expect(data.response).toBe(true);
  });

  for (const role of ["DocSpaceAdmin", "RoomAdmin"] as const) {
    test(`${role} can update public flag of own template`, async ({
      apiSdk,
    }) => {
      const { api } = await apiSdk.addAuthenticatedMember("owner", role);
      const templateId = await createTemplate(
        api,
        `Autotest SetPublic ${role} Own`,
      );

      const { status } = await api.rooms.setPublicSettings({
        setPublicDto: { id: templateId, public: true },
      });
      expect(status).toBe(200);
  ...
  for (const role of ["DocSpaceAdmin", "RoomAdmin", "User", "Guest"] as const) {
    test(`${role} cannot update public flag of Owner's template`, async ({
      apiSdk,
    }) => {
      const ownerApi = apiSdk.forRole("owner");
      const templateId = await createTemplate(
        ownerApi,
        `Autotest SetPublic ${role} OnOwner`,
      );
  ...
  test("Owner cannot update public flag of DocSpaceAdmin's template", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { api: adminApi } = await apiSdk.addAuthenticatedMember(
    ...
    const { data: flag } = await adminApi.rooms.getPublicSettings({
      id: templateId,
    });
    expect(flag.response).toBe(false);
    expect((data as any).statusCode).toBe(403);
  ...
  test("Invited source-room member cannot update template public flag", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
      createRoomRequestDto: {
        title: "Autotest SetPublic Invited Source",
        roomType: RoomType.CustomRoom,
      },
    });
    ...
    await ownerApi.rooms.createRoomTemplate({
      roomTemplateDto: {
        roomId: sourceRoomId,
        title: "Autotest SetPublic Invited Template",
      },
    });
    const templateId = await waitForRoomTemplate(ownerApi.rooms);
  ...
  test("Terminated user cannot update own template public flag", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: memberData, api: adminApi } =
    ...
    const { status } = await adminApi.rooms.setPublicSettings({
      setPublicDto: { id: templateId, public: true },
    });
    expect(status).toBe(401);
  });
  ...
  test("Unauthenticated request returns 401", async ({ apiSdk }) => {
    ...
    const templateId = await createTemplate(
      ownerApi,
      "Autotest SetPublic Anon",
    );

    const { status } = await apiSdk.forAnonymous().rooms.setPublicSettings({
      setPublicDto: { id: templateId, public: true },
    });
    expect(status).toBe(401);
  });
...
test.describe("POST /files/rooms/fromtemplate - access control", () => {
  ...
  test("BUG 81662: User cannot create a room from template (no create-room permission)", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
  ...
  test("BUG 81663: Guest cannot create a room from template", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
  ...
  test("BUG 81664: DocSpaceAdmin cannot create a room from a non-public template they don't own", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
  ...
  test("BUG 81662: User with source-room access but no template access cannot create room", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
  ...
  test("BUG 81662: User with template access still cannot create room without create-room permission", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    const { data: roomData } = await ownerApi.rooms.createRoom({
...
test.describe("PUT /api/2.0/files/rooms/{id}/links - external sharing restriction", () => {
  test("BUG 81840: PUT /api/2.0/files/rooms/{id}/links - Owner cannot create new external link when external sharing is restricted with Allow existing links", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
