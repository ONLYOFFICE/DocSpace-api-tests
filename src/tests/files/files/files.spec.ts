...
test.describe("GET /files/favorites/:fileId - Change favorite status", () => {
  ...
  test("BUG 82069: GET /files/favorites/:fileId - File in a Third-party room appears in Favorites after Mark as favorite", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
...
test.describe("POST /files/file/:fileId/recent - Add file to recent", () => {
  ...
  test("BUG 82069: POST /files/file/:fileId/recent - File in a Third-party room appears in Recent after opening", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    ...
    const { data: tpData } =
      await ownerApi.thirdPartyIntegration.saveThirdParty({
        thirdPartyRequestDto: {
          ...
          login: config.NEXTCLOUD_LOGIN,
          password: config.NEXTCLOUD_PASSWORD,
          customerTitle: "Autotest Recent TP " + apiSdk.faker.generateString(6),
          providerKey: "Nextcloud",
        },
      });
...
test.describe("GET /files/file/:fileId/trackeditfile - Track file editing", () => {
  ...
  test("GET /files/file/:fileId/trackeditfile - Owner tracks editing", async ({
    apiSdk,
  }) => {
    const ownerApi = apiSdk.forRole("owner");
    ...
    const { data: editData } = await ownerApi.files.openEditFile({ fileId });
